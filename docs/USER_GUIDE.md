...
本文档适用于“语义雷达——基于微调中文 BERT 情感分析与 BERTopic 语义聚类的校园舆情态势研判平台”的本地使用、竞赛演示和基础运维。项目面向高校管理人员、校园公共安全与民生服务分析人员，以及非商业教学、个人研究和数据可视化竞赛场景。
...
分析实验室不是独立系统，它复用“语义雷达”的监测项目、信号和事件数据：
...
进入“数据接入”，选择文件并提交。单个文件最大为 10 MB。优先导入由报修系统、意见箱、问卷后台或公开信息汇总批量导出的记录，不需要为了使用平台而手工逐条编写舆情。

页面可下载[空白 CSV 模板](../web/static/templates/campus-opinion-import-template.csv)。在 Excel 中填写后，请使用“另存为 → CSV UTF-8（逗号分隔）”导出，避免中文乱码。导入前仅保留研判所需字段，删除姓名、学号、手机号、账号和精确房间号等可识别信息。
