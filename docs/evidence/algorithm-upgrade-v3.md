...
| 趋势预测 | ARIMA、SIR 独立运行 | 混合预测：ARIMA/SIR 基线 + LSTM 讨论量预测 + 加权融合与 SIR 形状先验 | `models/hybrid_predictor.py`、`services/tasks.py::_hybrid_analysis` | `hybrid_arima_sir_lstm_v4` |
...
3. **可复现性**。BERTopic 固定 `random_state=42`；混合预测固定随机种子并按时间切分训练/验证（后 20% 窗口），验证阶段仅用训练前缀拟合基线和缩放，正式预测阶段再使用全部历史数据；BERT 微调脚本输出 `training_meta.json`（数据量、超参、逐 epoch 指标、最优 macro-F1）。
...
python scripts/finetune_bert_sentiment.py --train <数据文件> --model-name google-bert/bert-base-chinese --epochs 4

# 启用语义聚类（.env 已默认 CLUSTERING_ENGINE=auto）
python -m web
```

混合预测可在工作台的“高级实验模型”中选择，或通过分析任务触发：`POST /api/v1/analysis-jobs {"type": "hybrid"}`；`full` 也会包含该结果。本地 `inline` 模式会将这两种重分析放到进程内后台线程执行；它不能跨服务重启恢复，正式部署应使用 Redis/RQ。

## 验证记录

- 2026-09-19 本机 Conda `cv` 环境：`python -m unittest discover -s tests -q` **67/67 通过**。
- `tests/test_suanfa_algorithms.py` 覆盖引擎开关与回退、BERT 结果注入与证据结构、来源标签优先级、BERTopic 注入确定性向量后的语义分组与版本标记、聚类失败回退 n-gram、混合预测参考范围与最小样本约束。`tests/test_hybrid_correctness.py` 额外覆盖连续日补零、稀疏数据门槛、验证阶段只用训练前缀拟合和取消状态保护。
- 旧测试通过模块级 `setUpModule` 固定基线引擎（`SENTIMENT_ENGINE=lexicon`、`CLUSTERING_ENGINE=ngram`），断言契约不受 `.env` 影响。
- 此前 CI 覆盖率口径（仅 `test_v2_monitoring`）记录为 **81%（1013/1250）**；本次修改尚未重新测量覆盖率，不能把旧数字当作当前结果。CI 环境未安装 bertopic，相关路径由本机专项测试覆盖。

## 边界声明

- 情感模型准确率取决于训练数据分布；"校园测试集 94%" 类指标必须来自真实人工标注的留存集，验收前不得作为既成事实引用。
- BERTopic 的"主题连贯性提升"需要在与基线相同样本上做主题词人工对比后才能写进答辩材料。
- 混合预测的上下界是未校准的验证残差参考范围，不具有 95% 覆盖率保证；无独立验证窗口时不提供范围。预测不构成因果结论。
