...
TOPIC_TEMPLATES = [
    {
        "topic": "校园食品与环境安全",
        "keywords": ["食堂", "食品安全", "餐饮卫生", "异物", "饮用水", "环境卫生", "健康"],
        "hot_words": ["食堂卫生核查", "食品留样检测", "餐品异物投诉", "饮用水检修", "后厨检查",
                      "供应链复核", "窗口整改", "环境消杀", "检测结果公开", "学生健康提醒"],
    },
    {
        "topic": "校园治安与反诈",
        "keywords": ["校园安全", "反诈", "缴费", "陌生链接", "治安", "失物", "应急"],
        "hot_words": ["冒充辅导员诈骗", "虚假缴费通知", "反诈预警", "陌生链接核验",
                      "校园巡查", "夜间安全提醒", "应急通报", "止付流程", "失物招领", "安全演练"],
    },
    {
        "topic": "校园数字服务保障",
        "keywords": ["校园网", "教务系统", "一卡通", "认证服务", "网络故障", "信息中心", "系统维护"],
        "hot_words": ["校园网中断", "教务系统异常", "统一认证故障", "一卡通服务恢复", "设备切换",
                      "信息中心通报", "在线课程受影响", "宿舍网络检修", "服务状态验证", "系统维护公告"],
    },
    {
        "topic": "校园后勤与住宿服务",
        "keywords": ["宿舍", "供水", "供电", "维修", "洗浴", "快递", "后勤"],
        "hot_words": ["宿舍停水通知", "供电故障检修", "报修进度", "洗浴服务调整", "快递站拥堵",
                      "空调维修", "宿舍设施更新", "后勤回应", "施工噪声", "服务满意度"],
    },
    {
        "topic": "校园教学与交通服务",
        "keywords": ["课程", "考试", "图书馆", "校车", "通行", "场馆", "教学保障"],
        "hot_words": ["课程调整通知", "考试安排变更", "图书馆服务", "校车班次调整",
                      "校园通行提醒", "教学楼设备故障", "场馆开放时间", "无障碍服务", "意见反馈", "部门回应"],
    },
]
...
def generate_dataset(num_posts=2000, num_comments=8000, seed=42, reference_time=None):
    ...
    for i in range(num_posts):
        ...
        post = {
            ...
            "source": random.choice(["微博公开内容", "校园论坛", "校务公开", "新闻媒体"]),
            "data_kind": "synthetic",
        }
        posts.append(post)
